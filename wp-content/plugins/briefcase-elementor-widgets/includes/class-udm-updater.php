...
class Updraft_Manager_Updater_1_5 {
	...
	public function __construct($mothership, $muid = 1, $relative_plugin_file, $interval_hours = 24, $auto_backoff = true, $debug = false) {
		...
		add_action('admin_notices', array($this, 'admin_notices_briefcasewp'));
		
		add_filter('auto_update_plugin', array($this, 'auto_update_plugin'), 10, 2);
		
		// Add bew activate briefcase class on footer
		add_action( 'elementor/editor/footer', [ $this, 'footer_editor_briefcasewp' ] );
	}
			...
			if (is_array($result) && isset($result['body'])) {
				...
				if (empty($decoded)) {
					...
					if (isset($decoded->code) && 'OK' == $decoded->code) {
						...
						$option['password'] = base64_encode($_POST['password']);
	...
	/**
	 * WordPress action briefcasewp page
	 */
	public function admin_notices_briefcasewp() {
		if (!current_user_can('update_plugins')) return;
			$screen = get_current_screen();
			if ( $screen->parent_file == 'bew-templates' ) {
				if (!$this->is_connected()) {
					$this->admin_notice_not_connected();
				} 
			}
	}
	
	/**
	 * WordPress action elementor editor page
	 */
	public function footer_editor_briefcasewp() {
				...
				//Check if is confirm password is set.
				...
				if ( $this->is_connected() & ($check_password == "yes" or $check_password_confirm == "yes")) {
					echo "<script>
					jQuery(document).ajaxComplete(function() {
						jQuery('#elementor-template-library-modal').addClass('briefcasewp-active');
					});
					</script>";	
					...
					echo "<script>
					jQuery(document).ajaxComplete(function() {
						jQuery('#elementor-template-library-modal').addClass('briefcasewp-no-active');
					});
					</script>";
