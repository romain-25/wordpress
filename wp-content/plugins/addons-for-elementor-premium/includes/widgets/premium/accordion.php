...
class LAE_Accordion_Widget extends Widget_Base {
    ...
    protected function render() {
        ...
        $output = '<div class="lae-accordion lae-' . $settings['style'] . '" data-toggle="' . ($settings['toggle'] == 'yes' ? "true" : "false") . '" data-expanded="' . ($settings['expanded'] == 'yes' ? "true" : "false") . '">';
        ...
        foreach ($settings['panels'] as $panel) :
            ...
            $child_output = '<div class="lae-panel" id="' . $panel_id . '">';

            $child_output .= '<div class="lae-panel-title">' . esc_html($panel['panel_title']) . '</div>';

            $child_output .= '<div class="lae-panel-content">' . $this->parse_text_editor($panel['panel_content']) . '</div>';

            $child_output .= '</div><!-- .lae-panel -->';

            $output .= apply_filters('lae_accordion_item_output', $child_output, $panel, $settings);

        endforeach;

        $output .= '</div><!-- .lae-accordion -->';

        echo apply_filters('lae_accordion_output', $output, $settings);
