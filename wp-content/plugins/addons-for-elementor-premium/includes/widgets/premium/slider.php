...
Vestibulum dapibus nunc ac augue. Nam at tortor in tellus interdum sagittis. Maecenas tempus, tellus eget condimentum rhoncus, sem quam semper libero, sit amet adipiscing sem neque sed ipsum. Sed lectus. Quisque ut nisi.',
    ...
    protected function render() {
        ...
        $output = '<div class="lae-slider lae-container ' . esc_attr($settings['class'])
            . '" data-settings=\'' . wp_json_encode($slider_settings) . '\'>';

        $output .= '<div' . $dir . $style . ' class="lae-flexslider">';
        ...
        foreach ($settings['slides'] as $slide):
            ...
            if (!empty($slide['slide_content'])):
                ...
                $child_output = '<li class="lae-slide">';

                $child_output .= $this->parse_text_editor($slide['slide_content']);

                $child_output .= '</li>';

                $output .= apply_filters('lae_slider_item_output', $child_output, $slide, $settings);
        ...
        $output .= '</div><!-- .lae-flexslider -->';

        $output .= '</div><!-- .lae-slider -->';

        echo apply_filters('lae_slider_output', $output, $settings);
