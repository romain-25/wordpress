		...
		if ( '' !== $settings['link_to'] ) {			
			...
			if ( $settings['link_icon'] ) {
				$this->add_render_attribute( 'link', [
					'class'    => 'bdt-icon bdt-position-small bdt-position-' . esc_attr( $link_icon_position ),
	...
	protected function _content_template() {
		...
		<# if ( settings.image.url ) {
			...
			if ('' !== settings.link_to) {
				...
				if ('' !== settings.link_icon) {
					view.addRenderAttribute( 'link', 'class', ['bdt-icon', 'bdt-position-small', 'bdt-position-' + link_icon_position] );
