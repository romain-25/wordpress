...
class Testimonial_Carousel extends Widget_Base {
	...
	public function render_title( $post_id ) {
		...
		?>
		<h4 class="bdt-testimonial-carousel-title bdt-margin-remove-bottom" itemprop="name"><?php echo esc_attr(get_the_title( $post_id )); ?></h4>
		<?php
	}
	...
	public function render_rating( $post_id ) {
		...
		<ul class="bdt-rating bdt-rating-<?php echo get_post_meta( $post_id, 'bdthemes_tm_rating', true ); ?> bdt-grid bdt-grid-collapse" bdt-grid itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
