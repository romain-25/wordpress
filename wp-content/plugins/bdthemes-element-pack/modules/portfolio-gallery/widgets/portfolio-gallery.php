...
class Portfolio_Gallery extends Widget_Base {
	...
	private function register_section_controls() {
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .bdt-portfolio-gallery .bdt-gallery-item-link, {{WRAPPER}} .bdt-portfolio-gallery .bdt-gallery-item-link svg' => 'color: {{VALUE}};',
				],
			]
		);
	...
	public function render_header($skin = 'default') {
		...
		if ( $settings['show_filter_bar'] ) {
			$this->add_render_attribute('portfolio-wrapper', 'bdt-filter', 'target: #bdt-portfolio' . $this->get_id());
		}
